...
import { invitationPath } from './invitation-link.mjs';
...
function Admin() {
  ...
  const copy = async guest => {try {await navigator.clipboard.writeText(`${location.origin}${invitationPath(guest)}`);setToast(`Invitation link copied for ${guest.name}.`);}catch{setError('Could not copy. Open the invitation and copy its address from your browser.');}};
