...
import { ArrowRight, ArrowLeft, ArrowDown, Search, X, Heart, Flower2, Sparkles, Users, CalendarDays, Clock3, MapPin, Share2, Download, Check, LockKeyhole } from 'lucide-react';
...
function Florals({className=''}) {return <img className={`w-florals ${className}`} src="/burgundy-florals.webp" alt="" aria-hidden="true"/>;}
...
function Invitation({id,back}) {
  ...
  return <div className="atelier invitation-atelier"><Header groom={data?.settings.groom} back={back}/><main>
    ...
    {!data ? <div className="w-loading">{error?'Please contact the family for your invitation.':'Opening your invitation…'}</div> : <>
      ...
      <section className="w-invitation-closing"><Flourish/><p>{data.settings.closing}</p><span>{data.settings.host}</span><div className="w-guest-actions"><button onClick={share}><Share2 size={15}/> Share invitation</button><button onClick={()=>window.print()}><Download size={15}/> Save / print card</button></div>{shareStatus&&<p className="w-share-status" role="status"><Check size={13}/>{shareStatus}</p>}</section>
    </>}
