...
export async function invitationPage(request, { store, template }) {
  ...
  const [guest, settings] = id ? await Promise.all([store.get(`guests/${id}`), store.get('settings')]) : [null, null];
  ...
  const image = `${url.origin}/wedding-share-card-v2.jpg`;
  const meta = `
  ...
  `;
  const html = template.replace(/<title>[\s\S]*?<\/title>/i, '').replace(/<meta\s+name="description"[^>]*>/i, '').replace(/<head>(\s*<meta charset="UTF-8"\s*\/?>)?/i, (_, charset = '') => `<head>${charset}${meta}`);
  // Only the linked guest's preview is cached, keyed by its full URL. Guest APIs
  // remain uncached, so removal and invitation/RSVP changes still apply immediately.
  return new Response(request.method === 'HEAD' ? null : html, {headers:{...headers, 'Netlify-CDN-Cache-Control':'public, durable, max-age=60'}});
}
