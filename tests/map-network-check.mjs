...
try {
  ...
  await page.mouse.move(marker.x+65,marker.y+45,{steps:8});
  await page.mouse.up();
  assert.notEqual(await coordinateInput.inputValue(),beforeDrag);
  assert(await page.getByRole('button',{name:'Use this pin',exact:true}).isEnabled());
  ...
  await panel.getByRole('button',{name:'Adjust location pin'}).click();
  const savedCoordinates=await coordinateInput.inputValue();
  await page.getByRole('button',{name:'Manga Mandi',exact:true}).click();
  assert.equal(await coordinateInput.inputValue(),'');
  await page.getByRole('button',{name:'Cancel',exact:true}).click();
  await panel.getByRole('button',{name:'Adjust location pin'}).click();
  assert.equal(await coordinateInput.inputValue(),savedCoordinates);
  await page.getByRole('button',{name:'Cancel',exact:true}).click();
