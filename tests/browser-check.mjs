...
try {
  ...
  assert.equal(await page.getByRole('tab',{name:'Pin picker',exact:true}).getAttribute('aria-selected'),'true');
  ...
  const pasteLocation=async value=>{
    await coordinateInput.selectText();
    await coordinateInput.evaluate((element,text)=>{const data=new DataTransfer();data.setData('text',text);element.dispatchEvent(new ClipboardEvent('paste',{clipboardData:data,bubbles:true,cancelable:true}));},value);
  };
  ...
  assert.equal(await firstVenue.getByLabel(/^Location link/).inputValue(),'https://maps.app.goo.gl/nearby-hall');
  ...
  await page.getByText('31.299426, 74.070037',{exact:true}).waitFor();
  assert.equal(await coordinateInput.inputValue(),'31.299426, 74.070037');
  ...
  await page.getByText('31.300050, 74.070020',{exact:true}).waitFor();
